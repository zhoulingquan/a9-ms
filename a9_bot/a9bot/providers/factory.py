...
from a9bot.config.schema import Config, InlineFallbackConfig, ModelPresetConfig
...
def _make_provider_core(
    ...
    *,
    ...
    spec = find_by_name(resolved.provider) if resolved.provider else None
    backend = spec.backend if spec else "openai_compat"

    from a9bot.providers.openai_compat_provider import OpenAICompatProvider
    ...
    provider = OpenAICompatProvider(
        ...
        api_type=p.api_type if p and resolved.provider == "openai" else "auto",
    )

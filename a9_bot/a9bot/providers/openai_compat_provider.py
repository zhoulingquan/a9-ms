...
from a9bot.providers.base import LLMProvider, LLMResponse, ToolCallRequest
from a9bot.providers.openai_responses import (
    consume_sdk_stream,
    convert_messages,
    convert_tools,
...
if TYPE_CHECKING:
    ...
    from a9bot.providers.registry import ProviderSpec
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/HKUDS/a9bot",
    "X-OpenRouter-Title": "a9bot",
...
def _openai_compat_timeout_s() -> float:
    ...
    return _float_env("A9BOT_OPENAI_COMPAT_TIMEOUT_S", _OPENAI_COMPAT_REQUEST_TIMEOUT_S)
...
def _uses_openrouter_attribution(spec: "ProviderSpec | None", api_base: str | None) -> bool:
    """Apply A9Bot attribution headers to OpenRouter requests by default."""
...
def _openai_sdk_base_url(api_base: str | None) -> str | None:
    """Allow users to paste either a base URL or a full chat-completions URL."""
    if not api_base:
        return api_base
...
class OpenAICompatProvider(LLMProvider):
    ...
    def __init__(
        ...
        effective_base = _openai_sdk_base_url(effective_base)
    ...
    async def chat_stream(
        ...
        await self._ensure_client()
        idle_timeout_s = int(os.environ.get("A9BOT_STREAM_IDLE_TIMEOUT_S", "90"))
        try:
            if self._should_use_responses_api(model, reasoning_effort):
                try:
                    body = self._build_responses_body(

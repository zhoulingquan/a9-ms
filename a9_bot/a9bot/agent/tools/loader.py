...
def _tool_name_allowed(ctx: Any, name: str) -> bool:
    configured = getattr(ctx.config, "enabled_tools", ["*"])
...
class ToolLoader:
    def __init__(self, package: Any = None, *, test_classes: list[type[Tool]] | None = None):
        if package is None:
            import a9bot.agent.tools as _pkg
            package = _pkg
    ...
    def _discover_plugins(self) -> dict[str, type[Tool]]:
        ...
        try:
            eps = entry_points(group="a9bot.tools")
        except Exception:
            return plugins
        for ep in eps:
            try:
        ...
        for source, is_plugin_source in sources:
            for tool_cls in source:
                ...
                try:
                    ...
                    if not _tool_name_allowed(ctx, tool.name):
                        continue

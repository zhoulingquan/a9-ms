"""MCP client: connects to MCP servers and wraps their tools as native a9bot tools."""
...
from a9bot.agent.tools.base import Tool
from a9bot.agent.tools.registry import ToolRegistry
from a9bot.bus.events import (
...
class MCPToolWrapper(Tool):
    """Wraps a single MCP server tool as a a9bot Tool."""
...
class MCPResourceWrapper(Tool):
    """Wraps an MCP resource URI as a read-only a9bot Tool."""
...
class MCPPromptWrapper(Tool):
    """Wraps an MCP prompt as a read-only a9bot Tool."""
    ...
    for item in structured[:8]:
        ...
        if configured_server_names is not None and raw_name not in configured_server_names:
            lines.append(
                ...
                "tell the user to restart a9bot."
            )
            continue
        if connected_server_names is not None and raw_name not in connected_server_names:
            lines.append(
                ...
                "run the preset test, and restart a9bot only if hot reload is unavailable."
            )
            continue
        lines.append(
            "MCP Preset Attachment: "
...
async def reload_servers(state: Any, registry: ToolRegistry) -> dict[str, Any]:
    ...
    async with _reload_lock(state):
        try:
            ...
            return {
                "ok": False,
                "message": "Could not reload MCP config. Restart a9bot to pick up changes.",
        ...
        if failed:
            ...
            message = "MCP connections refreshed without restarting a9bot."
        else:
            message = "MCP config reloaded without restarting a9bot."
...
async def request_mcp_reload(bus: Any, *, timeout: float = 15.0) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "ok": False,
            "message": "MCP hot reload timed out. Restart a9bot to pick up changes.",
            "requires_restart": True,
        }
...
async def handle_runtime_control(state: Any, msg: InboundMessage, registry: ToolRegistry) -> bool:
    ...
    try:
        ...
        result = {
            "ok": False,
            "message": "MCP hot reload failed. Restart a9bot to pick up changes.",
            "requires_restart": True,
            "error": str(exc),
        }

...
from a9bot.utils.helpers import build_image_content_blocks
...
# Hostnames that are always private / internal (checked without DNS resolution).
_PRIVATE_HOSTNAMES: frozenset[str] = frozenset({
    "localhost", "localhost.localdomain",
    "host.docker.internal", "host.kubernetes.internal",
    "127.0.0.1", "::1", "0.0.0.0",
    "metadata.google.internal", "100.100.100.200",  # GCP/AWS/Alibaba metadata
})
...
_PRIVATE_IP_PREFIXES: tuple[ipaddress.IPv4Network, ...] = (
...
def _is_private_ip(ip_str: str) -> bool:
    ...
    if addr.is_loopback or addr.is_private or addr.is_link_local:
        return True
    if addr.version == 4:
        return any(addr in net for net in _PRIVATE_IP_PREFIXES)
    return False


async def _validate_url_safe(
    url: str,
    ssrf_whitelist: list[str] | None = None,
) -> tuple[bool, str]:
    """Validate URL with SSRF protection: scheme, domain, and resolved IP check.

    Returns (True, "") on success or (False, error_message) when the URL points
    to a private or internal address. Error messages include SSRF markers that
    the runner's ``_is_ssrf_violation`` detects to prevent retry loops.
    """
    ...
    # Fast-path check for known private hostnames (no DNS lookup needed).
    if hostname in _PRIVATE_HOSTNAMES:
        return False, f"Blocked private/internal address: {hostname}"
    ...
    try:
        ...
        # DNS resolution failure is not an SSRF signal — let the fetch
        # attempt fail naturally (the error will be clearer).
        return True, ""

    # Build whitelist networks for exemption checking.
    whitelist_nets: list[ipaddress.IPv4Network | ipaddress.IPv6Network] = []
    for cidr in (ssrf_whitelist or []):
        try:
            whitelist_nets.append(ipaddress.ip_network(cidr))
        except ValueError:
            logger.warning("Invalid CIDR in ssrf_whitelist: {}", cidr)
...
async def _get_with_safe_redirects(
    ...
    for _ in range(MAX_REDIRECTS + 1):
        is_valid, error_msg = await _validate_url_safe(current_url, ssrf_whitelist)
        if not is_valid:
        ...
        is_valid, error_msg = await _validate_url_safe(next_url, ssrf_whitelist)
        if not is_valid:
...
class WebSearchTool(Tool):
    ...
    def create(cls, ctx: Any) -> Tool:
        ...
        if ctx.provider_snapshot_loader is not None:
            def config_loader():
                from a9bot.config.loader import load_config, resolve_config_env_vars
...
class WebFetchTool(Tool):
    ...
    def create(cls, ctx: Any) -> Tool:
        return cls(
            ...
            proxy=ctx.config.web.proxy,
            user_agent=ctx.config.web.user_agent,
            ssrf_whitelist=ctx.config.ssrf_whitelist,
        )

    def __init__(self, config: WebFetchConfig | None = None, proxy: str | None = None, user_agent: str | None = None, max_chars: int = 50000, ssrf_whitelist: list[str] | None = None):
        self.config = config if config is not None else WebFetchConfig()
        self.proxy = proxy
        self.user_agent = user_agent or _DEFAULT_USER_AGENT
        self.max_chars = max_chars
        self.ssrf_whitelist = ssrf_whitelist or []
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        is_valid, error_msg = await _validate_url_safe(url, self.ssrf_whitelist)
        if not is_valid:

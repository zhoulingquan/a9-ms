...
from a9bot.config.loader import get_config_path, load_config, save_config
from a9bot.config.schema import ModelPresetConfig
# image_generation module deleted
def get_image_gen_provider(name): return None
def image_gen_provider_names(): return []
...
def _validate_configured_provider(
    ...
    *,
    ...
    provider_config = (
        config.get_provider(preset.model, preset=preset)
        if preset is not None
        else getattr(config.providers, provider, None)
    )
    if (
...
def settings_payload(*, requires_restart: bool = False) -> dict[str, Any]:
    ...
    # Only show custom provider — all others are stubs
    providers = []
    ...
    image_config = type("Dummy",(),{"enabled":False,"provider":"","model":"","default_aspect_ratio":"1:1","default_image_size":"1024x1024","max_images_per_turn":1,"save_dir":""})()
    search_provider = (
    ...
    def model_preset_api_fields(preset: ModelPresetConfig) -> dict[str, Any]:
        provider_config = config.get_provider(preset.model, preset=preset)
        provider_name_for_preset = config.get_provider_name(preset.model, preset=preset)
        spec_for_preset = find_by_name(provider_name_for_preset or preset.provider)
        return {
            "api_key_hint": _mask_secret_hint(
                getattr(provider_config, "api_key", None)
            ),
            ...
            "default_api_base": (
                spec_for_preset.default_api_base
                if spec_for_preset and spec_for_preset.default_api_base
                else None
            ),
            "api_type": getattr(provider_config, "api_type", "auto"),
        }
    ...
    model_presets = [
        {
            ...
            **model_preset_api_fields(default_preset),
        ...
        model_presets.append(
            {
                ...
                **model_preset_api_fields(preset),
    ...
    exec_config = type("Dummy",(),{"enable":False,"sandbox":None,"path_append":[]})()
    return {
        "agent": {
...
def create_model_configuration(query: QueryParams) -> dict[str, Any]:
    ...
    api_type = (_query_first_alias(query, "api_type", "apiType") or "auto").strip() or "auto"
    ...
    _validate_configured_provider(config, provider, preset=preset)

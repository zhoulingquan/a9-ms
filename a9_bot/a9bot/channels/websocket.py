...
from a9bot.agent.tools.mcp import request_mcp_reload
from a9bot.bus.events import OUTBOUND_META_AGENT_UI, OutboundMessage
from a9bot.bus.queue import MessageBus
from a9bot.channels.base import BaseChannel
from a9bot.command.builtin import builtin_command_palette
from a9bot.config.paths import get_media_dir, get_workspace_path
from a9bot.config.schema import Base
from a9bot.session.goal_state import goal_state_ws_blob
from a9bot.session.webui_turns import websocket_turn_wall_started_at
from a9bot.utils.helpers import safe_filename
from a9bot.utils.media_decode import (
    FileSizeExceeded,
    save_base64_data_url,
)
from a9bot.utils.subagent_channel_display import scrub_subagent_messages_for_channel
from a9bot.webui.settings_api import (
    WebUISettingsError,
...
# CLI apps and MCP presets removed — stubbed
async def _async_noop(*a, **kw): return {"presets": [], "cli_apps": []}
mcp_presets_settings_action = _async_noop
cli_apps_action = _async_noop
cli_apps_payload = lambda: {"cli_apps": []}
normalize_cli_app_mentions = lambda *a, **kw: []
normalize_mcp_preset_mentions = lambda *a, **kw: []
from a9bot.webui.sidebar_state import (
    read_webui_sidebar_state,
    write_webui_sidebar_state,
)
from a9bot.webui.thread_disk import delete_webui_thread
from a9bot.webui.transcript import (
...
def _issue_route_secret_matches(headers: Any, configured_secret: str) -> bool:
    ...
    header_token = (
        headers.get("X-A9Bot-Auth")
        or headers.get("x-a9bot-auth")
...
class WebSocketChannel(BaseChannel):
    ...
    async def _dispatch_http(self, connection: Any, request: WsRequest) -> Any:
        ...
        if got == "/api/skills":
            return self._handle_skills(request)

        if got == "/api/skills/create":
            return self._handle_skills_create(request)

        if got == "/api/settings/provider/test":
            return self._handle_provider_test(request)
    ...
    def _handle_skills(self, request: WsRequest) -> Response:
        if not self._check_api_token(request):
            return _http_error(401, "Unauthorized")
        from a9bot.webui.skills_api import list_skills
        return _http_json_response({"skills": list_skills(self._workspace_path)})

    def _handle_skills_create(self, request: WsRequest) -> Response:
        if not self._check_api_token(request):
            return _http_error(401, "Unauthorized")
        from a9bot.webui.skills_api import create_skill
        from a9bot.webui.settings_api import WebUISettingsError
        query = _parse_query(request.path)
        name = _query_first(query, "name") or ""
        content = (_query_first(query, "content") or "").strip()
        if not name or not content:
            return _http_error(400, "name and content are required")
        try:
            result = create_skill(self._workspace_path, name, content)
        except WebUISettingsError as e:
            return _http_error(e.status, e.message)
        return _http_json_response(result)

    def _handle_provider_test(self, request: WsRequest) -> Response:
        if not self._check_api_token(request):
            return _http_error(401, "Unauthorized")
        query = _parse_query(request.path)
        api_key = _query_first(query, "api_key") or ""
        api_base = _query_first(query, "api_base") or ""
        model = _query_first(query, "model") or "gpt-4o-mini"
        if not api_base:
            return _http_error(400, "API Base is required")
        import urllib.request as _ur
        import socket as _socket
        url = api_base.rstrip("/")
        if not url.endswith("/chat/completions"):
            url = url + "/chat/completions" if not url.endswith("/v1") else url + "/chat/completions"
        headers = {"Content-Type": "application/json"}
        if api_key:
            headers["Authorization"] = f"Bearer {api_key}"
        import json as _json
        body = _json.dumps({"model": model, "messages": [{"role": "user", "content": "hi"}], "max_tokens": 5})
        try:
            ...
            return _http_json_response({"success": False, "message": f"无法连接到服务器: {str(e)[:100]}"})
        try:
            import http.client as _hc
            parsed = _up.urlparse(url)
            is_https = parsed.scheme == "https"
            conn = _hc.HTTPSConnection(parsed.hostname, parsed.port or 443, timeout=15) if is_https else _hc.HTTPConnection(parsed.hostname, parsed.port or 80, timeout=15)
            path = parsed.path or "/"
            if parsed.query:
            ...
            conn.close()
            if resp.status == 200:
                return _http_json_response({"success": True, "message": "连接成功"})
            else:
                return _http_json_response({"success": False, "message": f"连接失败 (HTTP {resp.status})"})
        except Exception as e:
            return _http_json_response({"success": False, "message": f"连接失败: {str(e)[:150]}"})
    ...
    async def start(self) -> None:
        from a9bot.utils.logging_bridge import redirect_lib_logging

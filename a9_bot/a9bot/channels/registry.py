...
def load_channel_class(module_name: str) -> type[BaseChannel]:
    ...
    from a9bot.channels.base import BaseChannel as _Base

    mod = importlib.import_module(f"a9bot.channels.{module_name}")
    for attr in dir(mod):
        obj = getattr(mod, attr)
        if isinstance(obj, type) and issubclass(obj, _Base) and obj is not _Base:
            return obj
    raise ImportError(f"No BaseChannel subclass in a9bot.channels.{module_name}")
...
def discover_plugins(enabled_names: set[str] | None = None) -> dict[str, type[BaseChannel]]:
    ...
    for ep in entry_points(group="a9bot.channels"):

...
from a9bot import __version__
from a9bot.bus.events import OutboundMessage
from a9bot.command.router import CommandContext, CommandRouter
from a9bot.utils.helpers import build_status_content
from a9bot.utils.restart import set_restart_notice_to_env
...
async def cmd_restart(ctx: CommandContext) -> OutboundMessage:
    ...
    async def _do_restart():
        ...
        os.execv(sys.executable, [sys.executable, "-m", "a9bot"] + sys.argv[1:])
...
async def cmd_status(ctx: CommandContext) -> OutboundMessage:
    ...
    with suppress(Exception):
        from a9bot.utils.searchusage import fetch_search_usage
...
async def cmd_pairing(ctx: CommandContext) -> OutboundMessage:
    """List, approve, deny or revoke pairing requests."""
    # Pairing removed — A9MS uses session auth
    return OutboundMessage(
        channel=ctx.msg.channel,
        chat_id=ctx.msg.chat_id,
        content="Pairing is not available in A9MS mode.",
    )

...
from a9bot.cron.types import CronSchedule

if TYPE_CHECKING:
    from a9bot.integrations.a9ms.tools import A9MSToolsConfig
    from a9bot.agent.tools.self import MyToolConfig
    from a9bot.agent.tools.web import WebToolsConfig
...
class ModelPresetConfig(Base):
    ...
    def to_generation_settings(self) -> Any:
        from a9bot.providers.base import GenerationSettings
...
class AgentDefaults(Base):
    ...
    bot_name: str = "a9bot"  # Display name shown in CLI prompts (e.g. "{name} is thinking...")
...
class ToolsConfig(Base):
    ...
    web: WebToolsConfig = Field(default_factory=lambda: _lazy_default("a9bot.agent.tools.web", "WebToolsConfig"))
    a9ms: "A9MSToolsConfig" = Field(default_factory=lambda: _lazy_default("a9bot.integrations.a9ms.tools", "A9MSToolsConfig"))
    my: MyToolConfig = Field(default_factory=lambda: _lazy_default("a9bot.agent.tools.self", "MyToolConfig"))
    exec: "ExecConfig" = Field(default_factory=lambda: ExecConfig())
    restrict_to_workspace: bool = False  # restrict all tool access to workspace directory
    enabled_tools: list[str] = Field(default_factory=lambda: ["*"])  # "*" keeps the default tool set; [] disables all discovered tools
    mcp_servers: dict[str, MCPServerConfig] = Field(default_factory=dict)
    ssrf_whitelist: list[str] = Field(default_factory=list)  # CIDR ranges to exempt from SSRF blocking (e.g. ["100.64.0.0/10"] for Tailscale)


class Config(BaseSettings):
    """Root configuration for a9bot."""
    ...
    @staticmethod
    def _provider_with_preset_overrides(
        provider: ProviderConfig,
        preset: ModelPresetConfig,
    ) -> ProviderConfig:
        updates: dict[str, Any] = {}
        if preset.api_key is not None:
            updates["api_key"] = preset.api_key
        if preset.api_base is not None:
            updates["api_base"] = preset.api_base
        if preset.api_type != "auto":
            updates["api_type"] = preset.api_type
    ...
    def _match_provider(
        ...
        *,
        ...
        from a9bot.providers.registry import PROVIDERS, find_by_name
        ...
        if forced != "auto":
            ...
            if spec:
                ...
                if p:
                    return (self._provider_with_preset_overrides(p, resolved), spec.name)
                return None, None
            return None, None
        ...
        for spec in PROVIDERS:
            ...
            if local_fallback is None:
                local_fallback = (self._provider_with_preset_overrides(p, resolved), spec.name)
        if local_fallback:
    ...
    def get_api_base(
        ...
        *,
        ...
        from a9bot.providers.registry import find_by_name
    ...
    model_config = ConfigDict(env_prefix="A9BOT_", env_nested_delimiter="__")
...
def _resolve_tool_config_refs() -> None:
    ...
    like ``from a9bot.config.schema import ExecToolConfig`` continue to work.
    """
    import sys

    from a9bot.agent.tools.self import MyToolConfig
    from a9bot.agent.tools.web import WebFetchConfig, WebSearchConfig, WebToolsConfig
    from a9bot.integrations.a9ms.tools import A9MSToolsConfig

    # Re-export into this module's namespace
    mod = sys.modules[__name__]
    mod.A9MSToolsConfig = A9MSToolsConfig  # type: ignore[attr-defined]

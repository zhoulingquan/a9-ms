...
def runner_wall_llm_timeout_s(
    ...
    *,
    ...
    """Wall-clock cap for :class:`~a9bot.agent.runner.AgentRunner` when streaming an LLM.

    Returns ``0.0`` to disable ``asyncio.wait_for`` around the request when a sustained goal is
    active; ``None`` means use ``A9BOT_LLM_TIMEOUT_S``. Pass in-memory ``metadata`` when the
    caller already holds :attr:`~a9bot.session.manager.Session.metadata` for this turn.
    """

...
# Remove default handler and re-add with unified a9bot format
logger.remove()
...
from a9bot import __logo__, __version__
...
from a9bot.cli.stream import StreamRenderer, ThinkingSpinner
from a9bot.config.paths import get_workspace_path, is_default_workspace
from a9bot.config.schema import Config
from a9bot.utils.helpers import sync_workspace_templates
from a9bot.utils.restart import (
...
app = typer.Typer(
    ...
    help=f"{__logo__} a9bot - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from a9bot.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    if show_header:
        console.print()
        console.print(f"[cyan]{__logo__} a9bot[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} a9bot[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} a9bot v{__version__}")
        raise typer.Exit()
...
def onboard(
    ...
    """Initialize a9bot configuration and workspace."""
    from a9bot.config.loader import get_config_path, load_config, save_config, set_config_path
    from a9bot.config.schema import Config
    ...
    if wizard:
        from a9bot.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'a9bot onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'a9bot agent -m "Hello!"'
    gateway_cmd = "a9bot gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} a9bot is ready!")
    console.print("\nNext steps:")
    if wizard:
    ...
    console.print(
        "\n[dim]Want Telegram/WhatsApp? See: https://github.com/HKUDS/a9bot#-chat-apps[/dim]"
    )
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from a9bot.channels.registry import discover_all
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from a9bot.config.loader import load_config, resolve_config_env_vars, set_config_path
...
def _warn_deprecated_config_keys(config_path: Path | None) -> None:
    ...
    from a9bot.config.loader import get_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from a9bot.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show a9bot runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'a9bot-ai[api]'[/red]")
        raise typer.Exit(1)

    from loguru import logger

    from a9bot.api.server import create_app
    from a9bot.bus.queue import MessageBus
    from a9bot.providers.image_generation import image_gen_provider_configs
    from a9bot.session.manager import SessionManager

    if verbose:
        logger.enable("a9bot")
    else:
        logger.disable("a9bot")
...
def _run_gateway(
    ...
    *,
    ...
    from a9bot.agent.tools.cron import CronTool
    from a9bot.agent.tools.message import MessageTool
    from a9bot.bus.queue import MessageBus
    from a9bot.channels.manager import ChannelManager
    from a9bot.channels.websocket import publish_runtime_model_update
    from a9bot.cron.service import CronService
    from a9bot.cron.types import CronJob
    from a9bot.providers.factory import build_provider_snapshot, load_provider_snapshot
    from a9bot.providers.image_generation import image_gen_provider_configs
    from a9bot.session.manager import SessionManager

    port = port if port is not None else config.gateway.port

    console.print(f"{__logo__} Starting A9Bot gateway version {__version__} on port {port}...")
    ...
    from a9bot.agent.loop import UNIFIED_SESSION_KEY
    from a9bot.bus.events import OutboundMessage
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        from a9bot.utils.evaluator import evaluate_response
    ...
    # Heartbeat service removed for A9MS integration
    ...
    from a9bot.cron.types import CronJob, CronPayload
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show a9bot runtime logs during chat"),
):
...
def channels_status(
    ...
    """Show channel status."""
    from a9bot.channels.registry import discover_all
    from a9bot.config.loader import load_config, set_config_path
...
def plugins_list():
    ...
    from a9bot.channels.registry import discover_all, discover_channel_names
    from a9bot.config.loader import load_config
...
def status():
    """Show a9bot status."""
    from a9bot.config.loader import get_config_path, load_config

    config_path = get_config_path()
    config = load_config()
    workspace = config.workspace_path

    console.print(f"{__logo__} a9bot Status\n")

    console.print(f"Config: {config_path} {'[green]✓[/green]' if config_path.exists() else '[red]✗[/red]'}")
    console.print(f"Workspace: {workspace} {'[green]✓[/green]' if workspace.exists() else '[red]✗[/red]'}")

    if config_path.exists():
        from a9bot.providers.registry import PROVIDERS

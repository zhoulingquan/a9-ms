"""Hatch build hook that bundles the webui (Vite) into a9bot/web/dist.
...
- No-op when `webui/package.json` is absent (e.g. installing from an sdist that
  already contains a prebuilt `a9bot/web/dist/`).
- Skips when `A9BOT_SKIP_WEBUI_BUILD=1` is set.
- Skips when `a9bot/web/dist/index.html` already exists, unless
  `A9BOT_FORCE_WEBUI_BUILD=1` is set.
...
class WebUIBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict) -> None:  # noqa: D401
        ...
        if os.environ.get("A9BOT_SKIP_WEBUI_BUILD") == "1":
            self.app.display_info("[webui-build] skipped via A9BOT_SKIP_WEBUI_BUILD=1")
            return

        if not package_json.is_file():
            self.app.display_info(
                "[webui-build] no webui/ source tree, assuming prebuilt a9bot/web/dist/"
            )
            return

        force = os.environ.get("A9BOT_FORCE_WEBUI_BUILD") == "1"
        if index_html.is_file() and not force:
            self.app.display_info(
                f"[webui-build] reusing existing build at {dist_dir} "
                "(set A9BOT_FORCE_WEBUI_BUILD=1 to rebuild)"
            )
            return
        ...
        if runner is None:
            raise RuntimeError(
                ...
                "install one or set A9BOT_SKIP_WEBUI_BUILD=1 to bypass."
            )

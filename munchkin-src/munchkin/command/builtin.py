...
async def cmd_status(ctx: CommandContext) -> OutboundMessage:
    ...
    with suppress(Exception):
        ...
        if search_cfg is not None:
            provider = getattr(search_cfg, "provider", "auto")
            tavily_key = getattr(search_cfg, "tavily_api_key", "") or None
            usage = await fetch_search_usage(provider=provider, tavily_key=tavily_key)

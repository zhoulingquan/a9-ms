...
# A9 多用户隔离:用于从 session.metadata 解析 workspace scope
...
class Consolidator:
    ...
    def _store_for_session(self, session: Session) -> MemoryStore:
        """A9 多用户隔离:返回 session 对应 workspace 的 MemoryStore。

        若 session.metadata 携带 WORKSPACE_SCOPE_METADATA_KEY 且指向非默认 workspace,
        则创建临时 MemoryStore(指向用户专属 workspace);否则返回默认 store。
        """
        ...
        try:
            ...
            if scope.project_path == self.store.workspace:
                return self.store
            return MemoryStore(scope.project_path)
        except Exception:
            logger.debug("解析 session workspace scope 失败,回退默认 store")
            return self.store
    ...
    async def _consolidate_replay_overflow(
        ...
        # A9 多用户隔离:用 session 对应 workspace 的 store 写入摘要
        summary = await self.archive(chunk, store=self._store_for_session(session))
    ...
    async def archive(self, messages: list[dict], store: MemoryStore | None = None) -> str | None:
        ...
        try:
            ...
            effective_store.append_history(summary, max_chars=_ARCHIVE_SUMMARY_MAX_CHARS)
            # A9 偏好学习:从 summary 中提取用户偏好增量,合并到 USER.md
            # 失败不影响主归档流程(仅记录 debug 日志)
            if summary and summary != "(nothing)":
                try:
                    await self._extract_and_merge_preferences(summary, effective_store)
                except Exception:
                    logger.debug("偏好提取失败,不影响主流程", exc_info=True)
            return summary
        except Exception:
            logger.warning("Consolidation LLM call failed, raw-dumping to history")
            effective_store.raw_archive(messages)
            return None

    async def _extract_and_merge_preferences(
        self,
        summary: str,
        store: MemoryStore,
    ) -> None:
        """A9 偏好学习:从归档摘要中提取用户偏好,合并到 USER.md。

        流程:
            1. 读取当前 USER.md 内容
            2. 调用 LLM(用 preference_extractor.md 模板)输入 summary + 当前 USER.md
            3. LLM 输出新的偏好 bullets(已与现有去重)
            4. 将 bullets 追加到 USER.md 的 "### Learned Preferences" 章节
            5. write_user() 写回,git auto_commit 版本化

        若 LLM 返回 "(none)" 或空内容,则跳过(无新偏好)。
        """
        current_user_md = store.read_user() or ""
        try:
            response = await self.provider.chat_with_retry(
                model=self.model,
                messages=[
            ...
            logger.debug("偏好提取 LLM 调用失败", exc_info=True)
            return

        if response.finish_reason == "error":
            logger.debug("偏好提取 LLM 返回错误: {}", response.content)
            return

        new_prefs = (response.content or "").strip()
        if not new_prefs or new_prefs == "(none)":
            return

        merged = self._merge_preferences_into_user_md(current_user_md, new_prefs)
        if merged == current_user_md:
            return  # 无变化

        store.write_user(merged)
        try:
            if store.git.is_initialized():
                store.git.auto_commit("A9 偏好学习: 更新 USER.md")
        except Exception:
            logger.debug("USER.md git commit 失败", exc_info=True)
        logger.info("A9 用户偏好已更新到 {}", store.user_file)

    @staticmethod
    def _merge_preferences_into_user_md(current: str, new_bullets: str) -> str:
        """将新偏好 bullets 合并到 USER.md 的 '### Learned Preferences' 章节。

        - 若 USER.md 已有 '### Learned Preferences' 章节,追加 bullets(去重)
        - 若没有该章节但有 '## Preferences' 章节,在该章节内末尾新增 '### Learned Preferences'
        - 若连 '## Preferences' 都没有,在文件末尾追加 '## Preferences\\n\\n### Learned Preferences'
        - 已存在的相同 bullet 不重复添加
        """
        # LLM 无新偏好时的占位符,直接返回原文
        normalized = (new_bullets or "").strip()
        if not normalized or normalized == "(none)":
            return current
        # 标准化新 bullets:按行拆分,去除空行
        new_lines = [line.strip() for line in normalized.splitlines() if line.strip()]
        if not new_lines:
            return current
        ...
        # 找到指定章节的末尾(下一个同级或更高级 ## / ### 标题前,或文件末尾)
        def _find_section_end(start_idx: int, heading_prefix: str) -> int:
        ...
        # 情况 1:已有 ### Learned Preferences 章节,追加去重
        marker_idx = next(
        ...
        if marker_idx is not None:
            ...
            existing_bullets = {
                lines[i].strip()
                for i in range(marker_idx + 1, insert_idx)
                if lines[i].strip().startswith("- ")
            }
            to_add = [line for line in new_lines if line not in existing_bullets]
            if not to_add:
                return current
            # 找到最后一个非空行位置,在其后追加
            last_content_idx = insert_idx - 1
            while last_content_idx > marker_idx and not lines[last_content_idx].strip():
                last_content_idx -= 1
            new_lines_list = lines[:last_content_idx + 1]
            new_lines_list.append("")  # 空行分隔
            new_lines_list.extend(to_add)
            # 保留原 insert_idx 到末尾的内容(可能包含空行)
            tail = lines[insert_idx:]
            if tail and not tail[0].strip():
                # 避免 双空行
                tail = tail[1:] if (len(tail) > 1 and not tail[1].strip()) else tail
            new_lines_list.extend(tail)
            result = "\n".join(new_lines_list)
            return result + ("\n" if current.endswith("\n") else "")

        # 情况 2:有 ## Preferences 章节,在其末尾(下一个 ## 之前)插入 ### Learned Preferences
        pref_idx = next(
            (i for i, line in enumerate(lines) if line.strip() == "## Preferences"),
            None,
        )
        if pref_idx is not None:
            # ## 章节末尾 = 下一个 ## 之前(不包含 ###,因为 ### 是 ## 的子章节)
            insert_idx = len(lines)
            for i in range(pref_idx + 1, len(lines)):
                if lines[i].strip().startswith("## "):
                    insert_idx = i
                    break
            section_block = [ "", marker, "" ] + new_lines + [""]
            new_lines_list = lines[:insert_idx] + section_block + lines[insert_idx:]
            result = "\n".join(new_lines_list)
            return result + ("\n" if current.endswith("\n") else "")

        # 情况 3:无 ## Preferences 章节,在文件末尾追加
        section_block = [
            "",
            "## Preferences",
            "",
        ...
        new_lines_list = lines[:] + section_block
        result = "\n".join(new_lines_list)
        return result + ("\n" if current.endswith("\n") else "")
    ...
    async def compact_idle_session(
        ...
        async with lock:
            ...
            if archive_msgs:
                # A9 多用户隔离:用 session 对应 workspace 的 store 写入摘要
                summary = await self.archive(archive_msgs, store=self._store_for_session(session))

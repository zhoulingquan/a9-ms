...
class WebSocketChannel(BaseChannel):
    ...
    def __init__(
        ...
        *,
        ...
        # A9 多用户隔离:connection -> email_hash (MD5(email)[:12])
        # 由 agent-proxy.js 在 WS upgrade 时通过 query 参数传入
        self._conn_email_hash: dict[Any, str] = {}
    ...
    def _cleanup_connection(self, connection: Any) -> None:
        ...
        self._conn_email_hash.pop(connection, None)

    # -- A9 多用户 workspace 隔离 -----------------------------------------

    def _scaffold_user_workspace(self, email_hash: str) -> Path:
        """为 A9 用户创建专属 workspace 目录(首次连接时调用)。

        目录结构:
            {workspace}/users/{email_hash}/
            ├── AGENTS.md      (从默认 workspace 复制 A9 CRM 工作指南)
            ├── SOUL.md        (Munchkin 默认模板)
            ├── USER.md        (Munchkin 默认模板)
            ├── memory/
            │   ├── MEMORY.md
            │   └── history.jsonl
            ├── sessions/      (会话 JSONL)
        ...
        已存在的目录不会重复 scaffold(幂等)。
        """
        from munchkin.utils.helpers import sync_workspace_templates

        user_workspace = (self._workspace_path / "users" / email_hash).resolve(strict=False)
        # 幂等:目录已存在且包含 AGENTS.md 则跳过
        if user_workspace.is_dir() and (user_workspace / "AGENTS.md").exists():
            return user_workspace

        user_workspace.mkdir(parents=True, exist_ok=True)
        # 1. 创建默认 Munchkin 模板文件(SOUL.md/USER.md/MEMORY.md/memory/history.jsonl/skills/)
        try:
            sync_workspace_templates(user_workspace, silent=True)
        except Exception:
            self.logger.exception("sync_workspace_templates 失败: {}", user_workspace)
        # 2. 从默认 workspace 复制 A9 AGENTS.md(CRM 工作指南)
        default_agents = self._workspace_path / "AGENTS.md"
        target_agents = user_workspace / "AGENTS.md"
        if default_agents.is_file() and not target_agents.exists():
            try:
                ...
                self.logger.exception("复制 AGENTS.md 失败: {} -> {}", default_agents, target_agents)
        # 3. 创建 sessions 目录(Munchkin session JSONL 存放处)
        (user_workspace / "sessions").mkdir(exist_ok=True)

        self.logger.info("A9 用户 workspace 已创建: {}", user_workspace)
        return user_workspace
    ...
    async def _connection_loop(self, connection: Any) -> None:
        ...
        # A9 多用户隔离:从 query 参数读取 email_hash,首次连接时 scaffold 用户 workspace
        email_hash_raw = _query_first(query, "email_hash")
        email_hash = email_hash_raw.strip() if email_hash_raw else ""
        if email_hash:
            # 校验 hash 格式(MD5 前 12 位:hex 字符串)
            if not re.fullmatch(r"[0-9a-f]{12}", email_hash):
                self.logger.warning("email_hash 格式非法,已忽略: {}", email_hash[:32])
                email_hash = ""
            else:
                self._conn_email_hash[connection] = email_hash
                try:
                    self._scaffold_user_workspace(email_hash)
                except Exception:
                    self.logger.exception("scaffold 用户 workspace 失败: email_hash={}", email_hash)
    ...
    async def _dispatch_envelope(
        ...
        if t == "message":
            ...
            # A9 多用户隔离:若该连接携带 email_hash,覆盖 scope 为用户专属 workspace
            # 这会覆盖默认 workspace 和 envelope 中传入的 scope,确保不同用户隔离
            email_hash = self._conn_email_hash.get(connection)
            if email_hash:
                user_workspace = self._scaffold_user_workspace(email_hash)

...
async def fetch_search_usage(
    ...
    Args:
        provider: Provider name (e.g. "auto", "tavily").
        api_key:  API key for the provider (falls back to env vars).
        tavily_key: Tavily API key (used when provider is "auto" to check Tavily usage).
    ...
    if p == "auto" and tavily_key:
        return await _fetch_tavily_usage(tavily_key)
    else:
        # auto (no tavily key), bing_scrape, baidu_scrape, searxng, unknown — no usage API
